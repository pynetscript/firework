...
def edit_user(user_id):
    ...
    if request.method == 'POST':
        ...
        # Prevent superadmin from deleting themselves (if this were handled in the edit form, which it isn't directly here)
        # This check is more relevant for the delete_user route.
        # if user.id == current_user.id and request.form.get('action') == 'delete':
        #     flash("You cannot delete your own account.", 'error')
        #     app_logger.warning(f"Superadmin {current_user.username} attempted to delete their own account.")
        #     return redirect(url_for('admin.users_list'))
